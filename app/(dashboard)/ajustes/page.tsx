...
export default function AjustesPage() {
  ...
  const [tab, setTab] = useState<'grupos' | 'cuenta'>('grupos')
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-slate-400">Grupos de clase organizados por día</p>
              </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-2xl">
                ...
                <button onClick={() => setModal('new')} className="text-blue-600 text-sm font-medium hover:underline">+ Crear primer grupo</button>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {grupos.map(g => {
                  ...
                  return (
                    <div key={g.id}
                      className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 hover:shadow-md transition cursor-pointer"
                      onClick={() => setModal(g)}>
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <button onClick={e => { e.stopPropagation(); handleDelete(g) }}
                          className="p-1.5 hover:bg-red-50 rounded-lg text-slate-300 hover:text-red-400 transition">
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      </div>
                      <h3 className="font-semibold text-slate-800">{g.nombre}</h3>
                      {g.horario && <p className="text-xs text-slate-400 mt-1">{g.horario}</p>}
                      {g.maestra && <p className="text-xs text-slate-500 mt-0.5">Maestra: {g.maestra}</p>}
                    </div>
                  )
                })}
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input defaultValue="Florencia Nightingale — Escuela de Enfermería" readOnly
...
function GrupoModal({ grupo, onSave, onClose }: {
  ...
  const handleSubmit = () => {
    ...
    onSave({ nombre: nombre.trim(), dia, horario: horario || null, maestra: maestra || null, color: DIA_COLORS[dia]?.bg ?? '#94A3B8' })
  }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 animate-fade-in" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <input value={nombre} onChange={e => setNombre(e.target.value)} placeholder="Ej: Grupo Martes A"
              ...
              {DIAS.map(d => {
                ...
                return (
                  <button key={d} onClick={() => setDia(d)}
                    ...
                    style={dia === d ? { background: dc.bg, color: '#fff', borderColor: dc.bg } : { background: '#fff', color: '#64748B', borderColor: '#E2E8F0' }}>
                    {d}
                  </button>
                )
              })}
          ...
          <div>
            ...
            <input value={horario} onChange={e => setHorario(e.target.value)} placeholder="Ej: 9:00 - 14:00"
              className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">Maestra encargada</label>
            <input value={maestra} onChange={e => setMaestra(e.target.value)} placeholder="Nombre de la maestra"
              className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
